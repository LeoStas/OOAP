...
%
    ...
    {\bfseries Расписание} $(Schedule)$ & Содержит всю информацию о движении поездов между двумя станциями:
        %
        \begin{itemize}
            \item номера поездов, которые останавливаются на этих станциях;
            \item стоимость проезда на каждом поезде;
            \item дата и время прибытия каждого поезда на станции и отправки с них.
        \end{itemize}
        %
    \\
    \hline
    {\bfseries Список расписаний} $(SchedulesList)$ & Хранит расписания между всеми парами станций. \\
    \hline
    {\bfseries Место} $(Seat)$ & Место в поезде:
    %
    \begin{itemize}
        \item тип места (верхнее, нижнее или боковое);
        \item номер места в вагоне;
        \item номер вагона в поезде.
    \end{itemize}
     \\
    \hline
    {\bfseries Поезд} $(Train)$ & Хранит информацию о поезде:
        %
        \begin{itemize}
            \item номер поезда;
            \item тип поезда (фирменный~/ скорый~/ пассажирский);
            \item для каждой станции хранится список билетов, действующих на этой станции;
            \item список мест в поезде;
            \item список посещаемых станций;
            \item времена и даты прибытия и отправки от каждой станции.
        \end{itemize}
        %
    \\
    \hline
    ...
    \hline
    {\bfseries Билет} $(Ticket)$ & Содержит информацию о билете:
        %
        \begin{itemize}
            \item уникальный номер билета;
            \item дата и время отправления и прибытия;
            \item станции отправления и прибытия;
            \item уникальный номер места (т.е. тройка: номер поезда, номер вагона, номер места в вагоне);
            \item тип билета (плацкартный, купейный ($1$, $2$ и $3$ класса), сидячий, в мягкий вагон, в вагон люкс и т. п.);
            \item цена билета (зависит от стоимости маршрута следования, типа поезда, типа билета и типа места);
            \item статус билета (место свободно~/ забронировано~/ продано~/ передано для реализации в кассы на вокзалах);
            \item данные клиента, купившего или забронировавшего билет (если место продано или забронировано);
            \item время истечения срока бронирования билета (в течение трёх суток с момента бронирования, но не позднее двух
    ...
    \hline
    {\bfseries Заявка} $(Request)$ & Хранит информацию о заявке клиента:
        %
        \begin{itemize}
            \item сведения о клиенте;
            \item сведения об одном и более билетах (к одной заявке может относиться несколько билетов на одно
                и то же имя);
            \item общая стоимость билетов в заявке;
            \item время создания заявки;
            \item время оплаты;
            \item вид доставки (самовывоз/курьер);
            \item адрес доставки;
            \item стоимость доставки;
            \item статус заявки (новая~/ рабочая~/ оплаченная~/ аннулированная);
            \item время истечения <<срока годности>> заявки (по истечении $12$ месяцев с момента создания
                заявки данные автоматически удаляются из системы).
        \end{itemize}
        %
    \\
    \hline
    {\bfseries Дальность доставки} $(Distance)$ & Дальность доставки билета. Бывает центром, спальным районом и дальним пригородом. \\
    \hline
    ...
    \if false
    {\bfseries ФИО} $(Name)$ & ФИО клиента, указываемые им при регистрации в службе бронирования. Хранится в базе клиентов. \\
    \hline
    {\bfseries Телефон} $(PhoneNumber)$ & Телефонный номер клиента. Хранится в базе клиентов.\\
    \hline
    {\bfseries Адрес электронной почты} $(Email)$ & Адрес электронной почты клиента. Хранится в базе клиентов.\\
    \hline
    {\bfseries Паспортные данные} $(Passport)$ & Паспортные данные клиента. Хранятся в базе клиентов. \\
    \hline
    {\bfseries Логин} $(Login)$ & Уникальный логин клиента. Генерируется системой автоматически. Хранится в базе клиентов. \\
    \hline
    {\bfseries Пароль} $(Password)$ & Пароль клиента. Генерируется системой автоматически. Хранится в базе клиентов. \\
    \hline
    \fi
\end{longtable}
%
